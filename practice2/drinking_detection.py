...
if __name__ == "__main__":
    ...
    network = cv.dnn.readNetFromDarknet(cfgFile='./yolo3/yolov3.cfg',
                                        darknetModel='./yolo3/yolov3.weights')
    network.setPreferableBackend(cv.dnn.DNN_BACKEND_OPENCV)

    layer_names = network.getLayerNames()
    layer_names = [
        layer_names[i - 1] for i in network.getUnconnectedOutLayers()
    ]
    ...
    while True:
        ...
        box_indices = cv.dnn.NMSBoxes(boxes, confidences, CONFIDENCES,
                                      THRESHOLD)
        if len(box_indices) > 0:
            for box_index in box_indices:
                ...
                object_type_name = OBJECT_TYPE_NAMES[
                    object_type_indices[box_index]]
                ...
                color = [
                    int(c) for c in COLORS[object_type_indices[box_index]]
                ]
                cv.rectangle(frame, (x, y), (x + w, y + h), color, 2)
                cv.putText(
                    frame,
                    f"{OBJECT_TYPE_NAMES[object_type_indices[box_index]]}: {confidences[box_index]:.4f}",
                    (x, y - 5), cv.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            else:
                if person_box and cup_box:
                    person_bottom = person_box[
                        0], person_box[1] + person_box[3] // 2
                    person_top = person_box[
                        0], person_box[1] - person_box[3] // 2
                    ...
                    distance = euclidean_distance(*person_box[:2],
                                                  *cup_box[:2])
                    if distance < MINIMAL_DISTANCE:
                        cv.putText(frame, "I AM DRINKING", (H // 3, 40),
                                   cv.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0),
                                   2)
                    else:
                        cv.putText(frame, "I AM NOT DRINKING", (H // 3, 40),
